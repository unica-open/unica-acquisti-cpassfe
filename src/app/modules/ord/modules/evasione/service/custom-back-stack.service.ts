...
export const customStackOperations = {
    tab: {
        evasione: 'tab_evasione',
        dettaglio: 'tab_dettaglio',
        riepilogo: 'tab_riepilogo'
    },
    interactions: {
        evasione: {
            readMode: 'act_eva_read_mode',
            editMode: 'act_eva_edit_mode',
            save: 'act_eva_save'
        },
        destinatario: {
            open: 'act_dest_open',
        ...
        riepilogo: {
            open: 'act_riep_open',
            readMode: 'act_riep_read_mode',
            editMode: 'act_riep_edit_mode',
            save: 'act_riep_save',
            createNew: 'act_riep_new'
        }
    }
};
...
export class CustomBackStackService {
    ...
    onEditEvasioneActivation = new EventEmitter<string>();
    ...
    private static isOpen(operation: string): boolean {
        ...
        return operation === inter.destinatario.open || operation === inter.riga.open || operation === inter.impegni.open || operation === inter.riepilogo.open;
    }

    private static isNew(operation: string): boolean {
        const inter = customStackOperations.interactions;
        return operation === inter.destinatario.createNew || operation === inter.riga.createNew || operation === inter.impegni.createNew || operation === inter.riepilogo.createNew;
    }

    static isEdit(operation: string): boolean {
        const inter = customStackOperations.interactions;
        return operation === inter.destinatario.editMode || operation === inter.riga.editMode || operation === inter.impegni.editMode || operation === inter.riepilogo.editMode;
    }
