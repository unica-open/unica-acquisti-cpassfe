...
export class CpassValidators {

  static atLeastOneNotEmpty() {
    return (controlGroup: FormGroup) => {
      const valid = CpassValidators.checkFormGroup(controlGroup, field => field.value);
      if (!valid) {
        return {
          atLeastOneNotEmpty : {
  ...
  static atLeastOneNotValid() {
    return (controlGroup: FormGroup) => {
      const valid = CpassValidators.checkFormGroup(controlGroup, field => field.valid);
      if (!valid) {
        return {
          atLeastOneNotEmpty : {
            text : 'At least one value should be set'
    ...
    if (controls) {
      return Object.keys(controls).some(key => {
        ...
        if (field instanceof FormGroup || field instanceof FormArray) {
          return CpassValidators.checkFormGroup(field, operation);
        }
